...
SHORTCUT_NAME="Cart Hoarding"
APP_NAME="Cart Hoarding.app"
...
download_and_extract() {
    echo "Downloading Cart Hoarding..."
    if curl -L -o "$ZIP_FILE" "$DROPBOX_URL" 2>&1 | tee -a "$LOG_FILE"; then
        ...
        if unzip "$ZIP_FILE" -d "$TEMP_DIR" 2>&1 | tee -a "$LOG_FILE"; then
            ...
            mv "$TEMP_DIR/Cart_Hoarding/"* "$EXTRACT_PATH"
        else
...
# Function to close processes containing "Cart_Hoarding"
close_cart_hoarding() {
    # Get the script's own PID
    local self_pid=$$
    
    # Find processes with "Cart_Hoarding" in their name, exclude this script's PID, and extract their PIDs
    local sniper_pids
    sniper_pids=$(pgrep -f "/Cart_Hoarding/" | grep -v "$self_pid")
    
    if [ -n "$sniper_pids" ]; then
        echo "Closing the following processes with 'Cart_Hoarding':"
        echo "$sniper_pids"
...
display_message "This script will now download and install Cart Hoarding and create a desktop shortcut for it."
...
if installation_successful; then
    create_launch_script
    display_message "Cart Hoarding is now installed. You can launch it from the desktop shortcut."
else
