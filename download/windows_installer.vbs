...
Sub KillRunningAppProcesses()
    ...
    ' Loop through all processes and terminate those in "\Cart_Hoarding\" path
    For Each objProcess In colProcesses
        ...
        If Not IsNull(exePath) And exePath <> "" Then
            If InStr(LCase(exePath), "\cart_hoarding\") > 0 Then
                On Error Resume Next
...
Sub DownloadAndExtract()
    ...
    downloadAndExtractScript = _
        ...
        "    New-NetFirewallRule -DisplayName 'Allow Cart Hoarding App' -Direction Inbound -Program (Join-Path $fullExtractPath 'node_modules\electron\dist\electron.exe') -Action Allow -Profile Any" & vbCrLf & _
        "} catch {" & vbCrLf & _
...
Sub CreateDesktopShortcut()
    ...
    shortcutPath = shell.SpecialFolders("Desktop") & "\Cart Hoarding.lnk"
...
DisplayMessage "This script will now download and install Cart Hoarding and create a desktop shortcut for it. PowerShell will open and close during this process, which is normal."
...
If Not CheckForErrors() And InstallationSuccessful() Then
    ...
    DisplayMessage "Cart Hoarding is now installed. You can launch it from the desktop shortcut."
Else
